...
//SoftwareSerial ss(13,15); // The serial connection to the GPS device3rx1txesp
HTTPClient http;
...
int sensorState=0; //for waterlevel1
const int sensorPin2 = 0; //for waterlevel2
int sensorState2=0; //for waterlevel2
const int sensorPin3 = 2; //for waterlevel3
int sensorState3=0;//for waterlevel3
...
void loop() {
  if (j == 0)
     ...
     http.addHeader("Content-Type", "application/x-www-form-urlencoded");
     //Data to send with HTTP POST
     String httpRequestData2 = "token="+authReadingsArr+"&lat="+latarray+"&lng="+lngarray; 
 ...
 //{
   ...
   latarray = String(lat,6);
   lngarray = String(lng,6);
   Serial.println(latarray);
   Serial.println(lngarray);
   //Long and Lat Sending
   http.begin(serverName2);
   http.addHeader("Content-Type", "application/x-www-form-urlencoded");
   //Data to send with HTTP POST
